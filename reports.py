...
from db import get_flights_for_month
from keyboards import BTN_PREV_MONTH, BTN_THIS_MONTH, main_menu_keyboard
from formatters import fmt_month_report
from time_utils import current_year_month, previous_year_month
...
async def report_this_month(message: Message) -> None:
    ...
    logger.info("This-month report — user=%s  %d/%02d  flights=%d", user_id, year, month, len(flights))
...
async def report_prev_month(message: Message) -> None:
    ...
    flights = await get_flights_for_month(user_id, year, month)
    logger.info("Prev-month report — user=%s  %d/%02d  flights=%d", user_id, year, month, len(flights))
    await message.answer(fmt_month_report(flights, year, month), reply_markup=main_menu_keyboard())
